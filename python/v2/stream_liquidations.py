#!/usr/bin/env python3
"""
Stream Liquidations Example - Hyperliquid gRPC API v2
...
Usage:
  ...
  python stream_liquidations.py --block 831000000  # From specific block
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Stream liquidation events from Hyperliquid L1 gRPC API (v2)",
...
def extract_liquidations(data: bytes, verbosity: int) -> list:
    ...
    try:
        ...
        # Structure 1: {"block_number": ..., "events": [...]}
        if isinstance(fills_data, dict) and "events" in fills_data:
            events = fills_data.get("events", [])
            block_number = fills_data.get("block_number")
            block_time = fills_data.get("block_time")
...
def extract_single_liquidation(user_address: str, fill_data: dict,
                                block_number: int = None, block_time: str = None) -> dict:
...
def format_liquidation(liq: dict, verbosity: int, liq_num: int) -> str:
    ...
    if verbosity >= 1:
        ...
        if liq["block_number"]:
            lines.append(f"   Block: #{liq['block_number']:,}")
...
def main():
    ...
    print("Hyperliquid gRPC - Stream Liquidations Example (v2 API)")
    print("========================================================")
